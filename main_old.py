...
from utils import parse_slice, parse_list, parse_float_list, THREAD_POOL_SIZE, LARGE_CHUNK_SIZE, JAX_CHUNK_SIZE
...
def preprocess(
    ...
    # Step 1: Load all frames into CPU memory at once
    """Load and process an ND2 or TIF file in memory-efficient batches of 16,000 pages."""
    
    if n_pages is None:
        n_pages = get_total_pages(input_path, ext)
    t_size = n_pages // n_z

    n_large_chunks = (n_pages + LARGE_CHUNK_SIZE - 1) // LARGE_CHUNK_SIZE  # Ensure full coverage

    for large_chunk_idx in range(n_large_chunks):
        t_start_large = large_chunk_idx * LARGE_CHUNK_SIZE
        t_end_large = min(t_start_large + LARGE_CHUNK_SIZE, n_pages)
        large_chunk_size = t_end_large - t_start_large

        print(f"\nLoading pages {t_start_large}-{t_end_large} ({large_chunk_size} pages) into memory...")

        # Load data in batches of 16,000 pages at a time
        if ext == '.nd2':
            all_frames = load_nd2_chunk(input_path, t_start_large, t_end_large, x_range, y_range)
        else:  # .tif or .tiff
            all_frames = load_tif_chunk(input_path, t_start_large, t_end_large, x_range, y_range)

        assert all_frames.shape[0] == large_chunk_size, "Frame count mismatch"

        # Reshape into 5D array
        all_frames = all_frames.reshape(large_chunk_size // n_z, n_z, *all_frames.shape[1:])  # [T, Z, C, H, W]

        # Step 2: Process frames in JAX-sized chunks
        n_chunks = (large_chunk_size // n_z + JAX_CHUNK_SIZE - 1) // JAX_CHUNK_SIZE

        with ThreadPoolExecutor(max_workers=THREAD_POOL_SIZE) as pool:
            for chunk_idx in tqdm(range(n_chunks), desc=f"Processing chunks {large_chunk_idx+1}/{n_large_chunks}"):
                t_start = chunk_idx * JAX_CHUNK_SIZE
                t_end = min(t_start + JAX_CHUNK_SIZE, large_chunk_size // n_z)
                current_chunk_size = t_end - t_start

                # Extract chunk from preloaded data
                chunk_data = all_frames[t_start:t_end]
                print(f"Processing chunk {chunk_idx+1}/{n_chunks}, shape: {chunk_data.shape}")

                chunk_data = jnp.array(chunk_data)
                chunk_data = jnp.transpose(chunk_data, (0, 2, 1, 3, 4))  # [T, C, Z, H, W]
                ...
                for c in range(n_channels):
                    ...
                    if save_as == 'nrrd':
                        ...
                        for t_offset in range(current_chunk_size):
                            t = t_start_large // n_z + t_start + t_offset
        ...
        print(f"\nFinished processing pages {t_start_large}-{t_end_large}. Clearing memory...\n")
